...
#logger.addHandler(logging.FileHandler('pplm.log', 'a'))
...
def perturb_past(
    ...
    for i in range(num_iterations):
        ...
        if loss_type == PPLM_BOW or loss_type == PPLM_BOW_DISCRIM:
            ...
            #logger.info(" pplm_bow_loss: {}".format(loss.data.cpu().numpy()))
        ...
        if loss_type == 2 or loss_type == 3:
            ...
            #logger.info(" pplm_discrim_loss: {}".format(discrim_loss.data.cpu().numpy()))
            loss += discrim_loss
        ...
        if kl_scale > 0.0:
            ...
            #logger.info(" kl_loss {}".format(kl_loss.data.cpu().numpy()))
            loss += kl_loss

        loss_per_iter.append(loss.data.cpu().numpy())
        ##logger.info(" pplm_loss {}".format((loss - kl_loss).data.cpu().numpy()))
...
def get_classifier(
    ...
    if isinstance(class_label, str):
        if class_label in params["class_vocab"]:
            ...
            #logger.info("class_label {} not in class_vocab".format(class_label))
            #logger.info("available values are: {}".format(params["class_vocab"]))
            #logger.info("using default class {}".format(label_id))
...
def generate_text_pplm(
    ...
    for i in trange(length, ascii=True):
        ...
        if classifier is not None:
            ...
            #logger.info("unperturbed discrim loss {}".format(unpert_discrim_loss.data.cpu().numpy()))
        else:
            unpert_discrim_loss = 0
...
def run_pplm_example(
    ...
    if uncond:
        ...
        raw_text = cond_text
        while not raw_text:
            #logger.info("Did you forget to add `--cond_text`? ")
    ...
    for i, pert_gen_tok_text in enumerate(pert_gen_tok_texts):
        try:
            ...
            pass
            #logger.info("Ignoring error while generating perturbed text: {}".format(exc))
